...
def gconv2d_addbias(input, bias, nti=8):
    ...
    #bias_dim = bias.get_shape().as_list()[-1]
    #gindices  = np.reshape([nti*[x] for x in  range(bias_dim)], nti*bias_dim)
    #transformed_bias = tf.gather(bias, gindices)
    #assert((transformed_bias.get_shape()[-1] ==
    #input.get_shape()[-1])),'bias:{} input: {}'.format(transformed_bias.get_shape()[-1], input.get_shape()[-1])
    #return tf.nn.bias_add(input, transformed_bias)

    n, h, w, nc = input.get_shape().as_list()
    ni = nc/nti
    input_channel_split = tf.reshape(input, [n, h, w, ni, nti])
    out_flat = bias + input_channel_split
    out = tf.transpose(out_flat, [0, 1, 2, 4, 3])
    return tf.reshape(out, [n, h, w, nc])
